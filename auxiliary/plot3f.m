function out = plot3f(in, varargin)
% uses the plot3 function on a 3 x X or a X x 3 matrix 'in'
...
if nargout > 0
    if nargin > 1
        out = plot3(in(1,:), in(2,:), in(3,:), varargin{:});
    else
        out = plot3(in(1,:), in(2,:), in(3,:));
    end
else
    if nargin > 1
        plot3(in(1,:), in(2,:), in(3,:), varargin{:});
    else
