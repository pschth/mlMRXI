function [ mrxisetup_optim, cost_optim] = optimizeCoilRadiiFrobeniusCondition( mrxisetup, s_min, s_max, s_interSteps, maxIter, verbose, debug )
...
%     ri = [1 2];
    ...
    for iy = rd
        ...
        for ix = rd
            ...
            rdb(ri(1)) = ix;
            rdb(ri(2)) = iy;
            L_curr = getLeadfieldMatrixWithNewCoilRadii(mrxisetup, VS, rdb);
...
L_curr = getLeadfieldMatrixWithNewCoilRadii(mrxisetup, VS, r);
...
while true
   ...
   % determine stepsize with largest cost reduction within s_min and s_max
    ...
    for si = 1:s_interSteps
        L_curr = getLeadfieldMatrixWithNewCoilRadii(mrxisetup, VS, r-s_search(si)*r_grad);
...
function L = getLeadfieldMatrixWithNewCoilRadii(mrxisetup, VS, r)
    nCoils = mrxisetup.getNumberOfCoils();
