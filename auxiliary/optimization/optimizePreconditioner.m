function M = optimizePreconditioner( L, s_max, s_min, s_interSteps, verbose )
...
while true
    ...
    % calculate gradient
    for i = 1:nVox-1
        LiLitdlitMtMli = L(:,i)*L(:,i)'./litMtMli(i);
        LitMtM = L(:,i)'*MtM;
        for j = i+1:nVox
            LiLjt = L(:,i)*L(:,j)';
            LiLjtpLjLit = LiLjt + LiLjt';
            grad = grad + ...
                (LiLjtpLjLit./(LitMtM*L(:,j)) -...
                LiLitdlitMtMli - L(:,j)*(L(:,j)'./litMtMli(j)))./...
                (litMtMli(i)*litMtMli(j));
        end
    end
    grad = M*grad;
