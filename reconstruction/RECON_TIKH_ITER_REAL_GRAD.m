classdef RECON_TIKH_ITER_REAL_GRAD < handle
    % from:
    ...
    methods (Access = public)
        function o = RECON_TIKH_ITER_REAL_GRAD(L,b,res,voxDelIdx)
            % constructor
            %
            % INPUT:
            ...
            o.info.name       = 'RECON_TIKH_ITER_REAL_GRAD';
    ...
    methods (Access = private)
        ...
        function o = init(o)
            ...
            o.req.wReg                  = 0.5;                                  % wReg = 0 - 1; Weighting between l2 regularization of solution and l2 regularization of gradient of solution (0 -> full Tikhonov reg., 1 -> full gradient reg.)
        ...
        function o = reconstruct(o)
            ...
            if o.int.i == 0
                ...
                % check if weighting factor is valid
                if o.req.wReg < 0 || o.req.wReg > 1
                    error('wReg has to be in a range between 0 and 1.')
                end
                % initialize result
                o.int.result = o.aux.xInit;
                
%                 % normalize weighting vector
                o.req.Gamma = o.req.Gamma./norm(o.req.Gamma)*o.int.nVox;
                ...
                o.int.gradMatrix = getVolumetricGradientMatrix(o.int.res);
                o.req.Gamma = o.req.wReg * o.int.gradMatrix(~o.int.voxDelIdx,~o.int.voxDelIdx) + (1-o.req.wReg) * o.req.Gamma;
                o.req.Gamma = o.req.Gamma./norm(full(o.req.Gamma))*o.int.nVox;
            ...
            if o.req.deltaMin == 0
                ...
                while o.int.i < o.req.nIter && o.req.deltaMin < o.int.dev
                    ...
                    o.int.result = o.int.Const1 + o.int.Const2*o.int.result;
